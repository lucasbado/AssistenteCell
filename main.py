...
async def lifespan(app: FastAPI):
    ...
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.NORMAL, callback=agente_perfil.processar)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.NORMAL, callback=agente_inferencia.processar)
    
    # Agentes de Reflexo especializados por categoria
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.NORMAL and e.categoria == CategoriaEvento.NOTIFICACAO, callback=agente_reflexo.processar)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.NORMAL and e.categoria == CategoriaEvento.MEDIA, callback=agente_musica.processar)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.NORMAL and e.categoria == CategoriaEvento.APP_FOREGROUND, callback=agente_foco.processar)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.NORMAL and e.categoria == CategoriaEvento.SENSOR_SYSTEM_CONTEXT, callback=agente_contexto_sistema.processar)
    
    # Pipeline de Saída de Notificações (com gestão de interrupções)
    kernel.registrar(filtro=lambda e: e.categoria == CategoriaEvento.INTENCAO_NOTIFICACAO, callback=agente_gestor_interrupcoes.processar)
    kernel.registrar(filtro=lambda e: e.categoria == CategoriaEvento.NOTIFICACAO_PRONTA_PARA_ENVIO, callback=agente_notificacoes.processar)

    # Pipeline Cognitivo (Eventos gerados pelos reflexos)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.EVENTO_COMPLEXO, callback=agente_roteador_cognitivo.processar)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.INTENCAO_RACIOCINIO, callback=agente_raciocinio.processar)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.INTENCAO_PESQUISA, callback=agente_pesquisa.processar)

    # Pipeline de Saída e Memória (Agentes terminais ou passivos)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.RESULTADO_PESQUISA, callback=agente_raciocinio.sintetizar_com_pesquisa)
    kernel.registrar(filtro=lambda e: True, callback=agente_episodico.processar)

    # Pipeline de Comandos do Sistema
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.GERAR_RESUMO_PERFIL, callback=agente_sumarizador_perfil.processar)

    # Pipeline de Aprendizado (Feedback do usuário)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.FEEDBACK_USUARIO, callback=agente_aprendizagem.processar)

    # Pipeline de Agrupamento de Contexto (Memória de Trabalho)
    kernel.registrar(filtro=lambda e: e.acao == TipoAcao.NORMAL and e.categoria == CategoriaEvento.NOTIFICACAO, callback=agente_memoria_trabalho.processar)

    # --- INÍCIO DOS PROCESSOS EM BACKGROUND ---
